...
class _NextButton extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final cakeIndent = ref.watch(cakeIndentProvider);
    final cakeIndentManager = ref.read(cakeIndentProvider.notifier);
    ...
    final cakeSize = cakeIndent.cake.size == CakeSizeType.One.transText
        ? CakeSizeType.One.sizeType
        : cakeIndent.cake.size == CakeSizeType.Two.transText
            ? CakeSizeType.Two.sizeType
            : CakeSizeType.Three.sizeType;
            ...
            child: Row(
              children: [
                Column(
                  ...
                  children: [
                    Text(
                      "옵션 가격( 사이즈($cakeSize) + 데코(${cakeIndent.cake.decorations.length}가지) + 배송비 미포함 ) ",
                      style: getTextTheme(context).bold.copyWith(fontSize: 12, height: 1),
                    ),
                    Padding(
                      padding: const EdgeInsets.only(top: 8.0),
                      child: Text(
                        "= ${NumberFormat("#,###").format(cakeIndentManager.getTotalPrice(isDelivery: false))}원",
                        style: getTextTheme(context).bold.copyWith(fontSize: 18, height: 1),
                      ),
                    ),
                  ],
                ),
